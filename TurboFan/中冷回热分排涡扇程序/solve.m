...
%  fprintf(fid,'MECHN1=%8.2f   steps=%8f\r\n',MECHN1, steps);
 ...
 fclose(fid);	
  ...
  if MECHN1~=0
      if traditional==0
           fid = fopen('offdesign1.txt','at+');
      else
        fid = fopen('traditionoffdesign1.txt','at+');
      end
        fprintf(fid,'飞行高度=%8.2f     马赫数=%f\r\n',ALTITUDE,MACH);
        fprintf(fid,'No  N1    N2     推力         耗油率  燃油流量    涵道比   SRF  SRC  风扇裕度  增压级裕度  压气机裕度\r\n');  
        fprintf(fid,'%8.5f   %8.5f   %8.5f   %8.5f   %8.5f   %8.5f   %8.5f\r\n',XI(1),XI(2),XI(3),XI(4),XI(5),XI(6),XI(7));
        fclose(fid);
        if traditional==0
             fid = fopen('offdesign2.txt','wt+');
        else
        fid = fopen('traditionoffdesign2.txt','wt+');               
        end   
        fprintf(fid,'沿流程总温\r\n');
        fprintf(fid,'飞行高度=%8.2f     马赫数=%f\r\n',ALTITUDE,MACH);
        fprintf(fid,'No  Tino   Tfano   Tcomo    Tburno   Thto    Tlto    Tbpo   Tson    Tso\r\n');
        fclose(fid);
        if traditional==0
         fid = fopen('offdesign3.txt','wt+');   
        else
        fid = fopen('traditionoffdesign3.txt','wt+');
        end
        fprintf(fid,'沿流程总压\r\n');
        fprintf(fid,'飞行高度=%8.2f     马赫数=%f\r\n',ALTITUDE,MACH);
        fprintf(fid,'No  Pino   Pfano   Pcomo    Pburno   Phto    Plto    Pbpo   Pson    Pso\r\n');
        fclose(fid);
         if traditional==0
             fid = fopen('offdesign4.txt','wt+');
        else
        fid = fopen('traditionoffdesign4.txt','wt+');
        end
        fprintf(fid,'沿流程流量\r\n');
        fprintf(fid,'飞行高度=%8.2f     马赫数=%f\r\n',ALTITUDE,MACH);
        fprintf(fid,'No  Wfano   Wbpo   Wcomp    Wcompout    Wburno     Whto     Wlto\r\n');
        fclose(fid);
  end
   ...
   fclose(fid);  
    if traditional==0
         fid = fopen('offdesign2.txt','at+');
        else
   fid = fopen('traditionoffdesign2.txt','at+');
    end
   fprintf(fid,'%d %8.2f  %8.2f  %8.2f  %8.2f  %8.2f  %8.2f  %8.2f  %8.2f\r\n',kk,Tino,Tfano,Tcomo,Tburno,Thto,Tlto,Tson,Tso);
   fclose(fid); 
    if traditional==0
        fid = fopen('offdesign3.txt','at+');
        else
   fid = fopen('traditionoffdesign3.txt','at+');
    end
   fprintf(fid,'%d %8.2f  %8.2f  %8.2f  %8.2f  %8.2f  %8.2f  %8.2f  %8.2f\r\n',kk,Pino,Pfano,Pcomo,Pburno,Phto,Plto,Pson,Pso);
   fclose(fid); 
    if traditional==0
          fid = fopen('offdesign4.txt','at+');
        else
   fid = fopen('traditionoffdesign4.txt','at+');
    end
   fprintf(fid,'%d %8.2f  %8.2f  %8.2f  %8.2f  %8.2f  %8.2f  %8.2f\r\n',kk,Wfano,Wbpo,Wcomp,Wcompout,Wburno,Whto,Wlto);
   fclose(fid); 
